...
class EndpointResponse:
    def __init__(self, 
                 ...
                 url: str):
        self.status = status
        self.content_type = content_type or 'application/octet-stream'  # Default content type if none provided
        self.data = data
        self.url = url
        ...
        try:
            ...
            if isinstance(data, dict):
                ...
                if data:
                    ...
                    item_schemas = [SchemaExtractor.extract_schema(item, max_depth - 1) 
...
class MLBAPIExecutor:
    def __init__(self, batch_size: int = 5, timeout: int = 30, delay: float = 1.0, max_retries: int = 3):
    ...
    async def fetch_endpoint(self, url: str, params: Dict[str, str], retries: int = 0) -> EndpointResponse:
        try:
        ...
        try:
            ...
            if response.is_success():
                ...
                if "error" not in schema:
                    ...
                    logger.error(f"Schema extraction error for {name}: {schema['error']}")
        ...
        for name, task in tasks:
            try:
                ...
                if "error" not in result:  # Only add successful responses
                    ...
                    logger.warning(f"Skipping endpoint {name} due to error: {result['error']}")
        ...
        try:
            ...
            output = {
                "metadata": {
                    ...
                    "successful_endpoints": total_endpoints - len(self.failed_endpoints),
