...
class Message(BaseModel):
    ...
    type: Literal["text", "options", "selection"] = Field(..., description="Type of message")
    suggestions: Optional[List[str]] = Field(None, description="Optional suggestions for user interaction")

class UserStats(BaseModel):
    daysActive: int = Field(default=0, description="Number of days the user has been active")
    messagesExchanged: int = Field(default=0, description="Total number of messages exchanged")
...
class UserPreferences(BaseModel):
    ...
    notificationPreference: str = Field(
        default="Game Time Only",
        description="When to send notifications"
    )
    statsPreference: str = Field(
        default="Basic",
        description="Level of statistical detail preferred"
    )
...
class BaseballPreferences(BaseModel):
    favoriteHomeRun: Optional[str] = Field(None, description="User's favorite home run moment")
    ...
    @classmethod
    def parse_gemini_response(cls, response_text: str, current_preferences: 'BaseballPreferences') -> 'BaseballPreferences':
        """
        ...
        try:
            ...
            new_prefs = cls(
                favoriteHomeRun=response_data.get('favoriteHomeRun', current_preferences.favoriteHomeRun),
                favoritePlayer=response_data.get('favoritePlayer', current_preferences.favoritePlayer),
                favoriteTeam=response_data.get('favoriteTeam', current_preferences.favoriteTeam),
                preferences=UserPreferences(
                    language=response_data.get('preferences', {}).get('language', current_preferences.preferences.language),
                    notificationPreference=response_data.get('preferences', {}).get('notificationPreference', 
                                                                                  current_preferences.preferences.notificationPreference),
                    statsPreference=response_data.get('preferences', {}).get('statsPreference', 
                                                                           current_preferences.preferences.statsPreference)
                ),
                stats=UserStats(
                    daysActive=response_data.get('stats', {}).get('daysActive', current_preferences.stats.daysActive),
                    messagesExchanged=response_data.get('stats', {}).get('messagesExchanged', 
                                                                       current_preferences.stats.messagesExchanged),
                    queriesAnswered=response_data.get('stats', {}).get('queriesAnswered', 
                                                                     current_preferences.stats.queriesAnswered)
                )
            )
            return new_prefs
...
class UpdateUserDataRequest(BaseModel):
    ...
    preferences: BaseballPreferences = Field(..., description="Current baseball preferences")
    ...
    async def analyze_with_gemini(self) -> BaseballPreferences:
        try:
            ...
            updated_prefs = BaseballPreferences.parse_gemini_response(response.text, self.preferences)
            
            # Update message count
            updated_prefs.stats.messagesExchanged = self.preferences.stats.messagesExchanged + len(self.messages)
    ...
    def update_stats(self) -> UserStats:
        ...
        new_queries = len([
            ...
            if msg.sender == "user" and 
            (msg.type == "selection" or any(s in msg.content.lower() for s in ["yes", "no", "choose"]))
        ])
